...
func newHandler(
	...
	root.Handle("/api/", buildAPIHandler(pages, applicationConfig, applicationMetrics))
	root.HandleFunc("GET /openapi.json", func(response http.ResponseWriter, _ *http.Request) {
		specification, err := gen.GetSpecJSON()
		if err != nil {
			http.Error(response, "unable to load API specification", http.StatusInternalServerError)
			return
		}

		response.Header().Set("Content-Type", "application/json")
		_, _ = response.Write(specification)
	})
